...
> **Rev 6.11 — 17 Aug 2026.** Supersedes Rev 6.10 (Tina): the smart-query UI is DROPPED — "UI is for simple visualisation"; smart/composable querying happens by pointing a Claude session at the store directly (read-only SQL over store.db, FTS included). FR-16 reverts to the verified simple search. New FR-33: a query-access doc (`QUERYING.md`) equips any Claude session to query smartly — schema map with key joins, FTS5 column-scoped syntax, arm/tier vocabulary, provenance flags, worked examples, read-only access rule.
> **Rev 6.10 — 17 Aug 2026 (superseded by 6.11).** FR-16 upgraded from filter search to **smart query** (Tina): field-scoped (title only / captured content only / both — FTS5 column filters), site-scoped (any subset of sites: one rival, the brand, several), fully composable with the existing taxonomy facets plus ranked/inventory facets (arm tags, capture status), and results show WHERE each match hit (title vs highlighted content snippet). Captured pages get their <title> extracted and indexed at capture time. Queued behind the MECE/comparison package; ships as the first M4/UI iteration item.
...
| FR-16 | P0 | Search box (FTS) + taxonomy filters; results link to stored versions. The UI search stays SIMPLE by design (Rev 6.11) — complex/composable querying is a Claude-session job over the store per FR-33 | Competitor slogan search lands on their stored page |
