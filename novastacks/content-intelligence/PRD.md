...
| FR-16 | P0 | **Smart query search** (Rev 6.10): FTS query with user-selectable FIELD scope (title only / captured content only / both — FTS5 column filters over an index carrying both the extracted page `<title>` and the markdown), SITE scope (any subset: one rival, the brand, several), composable with taxonomy facets and the ranked/inventory facets (arm tags, capture status) so queries like "content matches X on stridec + siegemedia, pages that rank on our keywords" resolve; results show where the match hit (title vs highlighted content snippet) and link to stored versions | Competitor slogan search lands on their stored page; a title-scoped query does not match content-only hits; a two-site scope returns only those sites; the match-location marker is visible per result |
