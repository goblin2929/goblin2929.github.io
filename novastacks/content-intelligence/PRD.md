...
> **Rev 6.8 — 17 Aug 2026.** Default capture-priority ordering added to FR-5 (Tina): for big-inventory rivals, full-content capture walks the union metrics — P1 both arms → P2 arm-1 only (our keywords; strategically closest even at low traffic) → P3 arm-2 only by est. traffic → P4 gap pages on demand / via approved-topic relevance. Default, overridable per pull.
...
| FR-5 | P1 | Deterministic selector, four signals in priority order: AI-cited (always in) → est.-traffic coverage to ~80% cumulative (configurable) → top-10 on client keyword panel → strategic URL patterns; every URL carries a machine-written reason; no LLM. **Default capture-priority ordering (Rev 6.8)** for a rival's content pull, derived from the FR-23 union: P1 = pages in BOTH arms (rank for the client's core keywords AND among the rival's top-traffic pages); P2 = arm-1 only (rank for the client's core keywords — strategically closest even at negligible traffic); P3 = arm-2 only, est.-traffic descending; P4 = gap pages (inventory-only), captured on demand or when an approved suggested topic needs its supporting pages. Default ordering, overridable per pull (CLI flag / UI curation action) | Fixed API fixtures → identical list on repeated runs; no row lacks a reason |
